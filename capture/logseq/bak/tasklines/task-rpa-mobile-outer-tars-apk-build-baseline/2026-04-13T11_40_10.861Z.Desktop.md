...
- ccos_node: `outer`
- status: `in_progress`
- updated_at: `2026-04-09 19:47 +0800`
...
11. 已补齐“正式 release 真机安装验证”当前结论：`v2.1.5` 正式 release APK 可从标签态成功重建；`s2` 当前安装的是 debug 签名 `2.1.5`；覆盖安装失败根因是 debug/release 签名不兼容（`INSTALL_FAILED_UPDATE_INCOMPATIBLE`），不是 release 构建流程本身失败。
...
4. 若需要完成“正式 release 真机安装验证”闭环，优先准备干净设备；若只能复用现有 debug 设备，则先走卸载影响确认，再执行正式安装与版本校验。

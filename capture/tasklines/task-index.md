# 任务线索引

> 说明：本文件是跨项目任务路由入口，项目内细节请链接到对应项目 `CCOS/context/task-*.md`。
> 并发模式下，Node 路由使用 `task-index + agent-focus`，Hub 不维护 `current-task`。

- updated_at: 2026-04-13 19:38 +0800

| taskline_id | 标题 | 状态 | 优先级 | project_id | ccos_node | 任务文档 | 来源 CCOS |
|---|---|---|---|---|---|---|---|
| `ccos/default/remote-control` | 手机跨网络远程控制 PC Codex 任务 | `in_progress` | `P0` | `ccos` | `default` | `task-ccos-default-remote-control.md` | `ccos/CCOS` |
| `ccos/default/dedup-daily-automation-v1` | CCOS 去冗余改造与自动日报闭环 V1 | `in_progress` | `P0` | `ccos` | `default` | `task-ccos-default-dedup-daily-automation-v1.md` | `ccos/CCOS` |
| `rpa-mobile/outer/xpeng-unified-framework-phase1` | 小鹏汽车项目通用流程框架 Phase 1 | `in_progress` | `P0` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-xpeng-unified-framework-phase1.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/xpeng-videohao-human-pick-phase1` | 小鹏视频号人类拾取与实现策略确认 Phase 1 | `in_progress` | `P0` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-xpeng-videohao-human-pick-phase1.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/xpeng-gongzhonghao-phase1` | 小鹏公众号任务实现 Phase 1 | `in_progress` | `P0` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-xpeng-gongzhonghao-phase1.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/xpeng-stable-baseline` | 小鹏稳定基线任务线 | `in_progress` | `P0` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-xpeng-stable-baseline.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/rpa-dev-platform-multi-ai-device-binding-phase1` | RPA 开发平台多 AI 并行设备绑定与长期上下文接入 Phase 1 | `in_progress` | `P0` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-rpa-dev-platform-multi-ai-device-binding-phase1.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/rpa-dev-platform-export-iteration` | RPA 开发平台导出流程包能力迭代 | `in_progress` | `P1` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-rpa-dev-platform-export-iteration.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/tars-diagnostic-logging-mvp` | 塔斯 App 诊断日志与平台态可观测性 MVP | `in_progress` | `P0` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-tars-diagnostic-logging-mvp.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/tars-commander-retry-hardening` | 塔斯 App commander 弱网重试与结果补偿加固 | `in_progress` | `P0` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-tars-commander-retry-hardening.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/tars-weak-network-result-delivery-phase1` | 塔斯 App 弱网结果上报稳态优化 Phase 1 | `in_progress` | `P0` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-tars-weak-network-result-delivery-phase1.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/tars-apk-build-baseline` | 塔斯 APK 打包命名与安装基线 | `in_progress` | `P1` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-tars-apk-build-baseline.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/tars-floating-window-permission-stability` | 塔斯 App 悬浮窗权限漂移与预热治理 | `planned` | `P1` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-tars-floating-window-permission-stability.md` | `apps/rpa-mobile/CCOS` |
| `wanling-tower/default/gameplay` | 万灵塔玩法重基线推进 | `in_progress` | `P1` | `wanling-tower` | `default` | `待创建` | `apps/game_demo/wanling-tower/CCOS` |
| `myth-td-prototype/default/immersion` | myth-td 沉浸感迭代 | `in_progress` | `P1` | `myth-td-prototype` | `default` | `待创建` | `apps/game_demo/myth-td-prototype/CCOS` |
| `rpa-mobile/outer/tars-accessibility-stability-phase1` | 塔斯 App 无障碍稳定性与自动重连优化 Phase 1 | `planned` | `P0` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-tars-accessibility-stability-phase1.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/tars-android-runtime-optimization-phase1` | 塔斯 App Android 子模块运行时轮询与上报优化 Phase 1 | `in_progress` | `P0` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-tars-android-runtime-optimization-phase1.md` | `apps/rpa-mobile/CCOS` |
| `rpa-mobile/outer/tars-screenshot-runtime` | 塔斯 App 截图链路重构与真机验证（Origin） | `done` | `P0` | `rpa-mobile` | `outer` | `task-rpa-mobile-outer-tars-screenshot-runtime.md` | `apps/rpa-mobile/CCOS` |
| `game_service/default/bootstrap` | game_service CCOS 联邦接入 | `done` | `P1` | `game_service` | `default` | `task-game-service-federation-migration.md` | `game_service/CCOS` |
| `ccos/default/unification-migration-20260303` | CCOS 中枢全量迁移与文档同步 | `done` | `P0` | `ccos` | `default` | `task-ccos-hub-unification-migration-20260303.md` | `ccos/CCOS` |

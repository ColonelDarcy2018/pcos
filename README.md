# CCOS（Cognitive Collaboration Operating System）

这是一个可版本控制、可回滚、可扩展的个人认知基础设施目录。它把日常思考与长期资产分离，并为未来自动化预留机器可读层。

CCOS 的定位是一个**认知进化系统**：结构的目的在于支持知识从经验到抽象的演化与复利，而不是把内容“分到正确的分类里”。
...
- `capture/`：原始捕获（建议作为 Logseq 图谱根目录；这里是“收件箱”）
  - `tasklines/`：中枢任务线（跨项目任务背景、推进状态、待确认项）
...
- `machine/`：机器可读结构（索引/Schema/图谱/提示词/Agent 定义）
  ...
  - `federation/`：多项目联邦索引（项目注册表、CCOS 聚合索引、联邦脚本）
...
为保证每天工作与想法都形成可追踪沉淀，CCOS 增加了项目化日报技能：
...
CCOS 作为全局中枢，采用“联邦模式”管理多项目：

- 项目内保留各自 `CCOS`（业务逻辑、架构、决策、上下文细节）。
- 中枢只维护索引与任务路由（不复制项目全量业务文档）。
- 通过注册表统一纳管项目及其 CCOS 节点（支持 mono 仓的外层/内层节点区分）。
...
1. 在项目目录执行 `ccosctl node ...`。
2. 涉及中枢治理时执行 `ccosctl hub ...`，可显式传 `--hub-root /Users/zhuxiaowei/ccos`。
3. 各项目 `CCOS/protocol/{p0-rules,ai-playbook}.md` 已强制锚定中枢协议：
